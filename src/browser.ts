...
export class SDNBrowser {
  ...
  public static async init(account: TwitterAccount) {
    ...
    const browser = await SDNBrowser.getBrowser(userDataDirectory, account)
    return new SDNBrowser(browser)
  }

  public async newPage() {
    return SDNBrowser.newPage(this.browser)
  }
  ...
  private static async login(page: Page, account: TwitterAccount) {
    const logger = Logger.configure('SDNBrowser.login')
